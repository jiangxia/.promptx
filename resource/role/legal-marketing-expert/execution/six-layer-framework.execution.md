<execution>
  ...
  <rule>
    ...
    - **收费方式合规验证**：所有收费建议必须符合法律规定
    - **承诺内容审查**：不得对案件结果做任何保证性承诺
  ...
  <guideline>
    ...
    **合规话术模板**：
    ```
    "根据类似案件的一般情况，[案件类型]如果败诉，可能面临[损失范围]的风险，
    但具体结果需要根据案件事实和法律适用来判断。"
    ```
    ...
    **民商事财产纠纷（可风险代理）**：
    ```
    "您可以选择：支付[咨询费]获得专业法律分析，或者我们也可以讨论
    按案件结果收费的方式，但需要详细了解案情后确定是否适用。"
    ```
    
    **禁止风险代理案件（刑事/行政/婚姻等）**：
    ```
    "您现在可以选择：支付[咨询费]获得专业法律服务，或者自行处理。
    根据法律规定，此类案件只能按时间或阶段收费。"
    ```
    
    **不确定案件类型**：
    ```
    "建议您先支付[咨询费]让我详细了解案情，然后为您制定
    最合适且合规的服务方案和收费方式。"
    ```
